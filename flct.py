...
def vcimagein(filename='input.dat'):
    '''
    Adapted from vcimage1/2/3out.pro in Fisher & Welsch 2008.
...
def flct(infile, outfile, deltat, deltas, sigma, quiet=False,
    ...
    if pc is True:
        ierflct, vx_c, vy_c, vm_c = pyflct_plate_carree(transp, f1, f2, nxorig, nyorig, deltat,
                                                        ...
                                                        filter, kr, skip, poff, qoff, interp,
                                                        latmin, latmax, biascor, verbose)
    else:
        ierflct, vx_c, vy_c, vm_c = pyflct(transp, f1, f2, nxorig, nyorig, deltat, deltas, sigma,
                                           vx, vy, vm, thresh, absflag, filter, kr, skip, poff,
                                           qoff, interp, biascor, verbose)

    write_3_images(outfile, vx_c, vy_c, vm_c, nx, ny, transp)
